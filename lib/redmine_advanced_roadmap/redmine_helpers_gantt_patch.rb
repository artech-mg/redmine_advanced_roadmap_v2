      ...
      base.class_eval do
        ...
        def render_version_with_milestones(project, version, options = {})
          if @last_rendered_project != project and project.milestones.any?
            subject_for_milestones_label(options, project)
        ...
        def render_milestone(project, milestone, options = {})
          # Milestone header
          subject_for_milestone(milestone, options, project) unless options[:only] == :lines
          line_for_milestone(milestone, options) unless options[:only] == :subjects
          options[:top] += options[:top_increment]
          @number_of_rows += 1
        end

        def subject_for_milestones_label(options, prj)
          case options[:format]
          when :html
        ...
        def subject_for_milestone(milestone, options, prj)
          case options[:format]
          when :html
